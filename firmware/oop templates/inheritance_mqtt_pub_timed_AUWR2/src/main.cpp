...
*   intended for commercial use. This code is a template for event  *
...
/*Function for inserting main functionallity of the device.
  This function is called in void loop()
  at the right time in the process. NOTE: data to be transmitted 
  to the MQTT-broker must be stored int the global var named "data"
  (int) */
void mainFunctionallity()
{
  /***********************************************
  ...
  ***********************************************/
  ...
  /* Member function call in instanciated objective
    for sending data to MQTT-broker */
  controllerObj.m_setData(sensorData);
}
