...
void loop() {
  ...
  for(int i=0;i < 16; i += 1){
      ...
      TF_LITE_REPORT_ERROR(error_reporter, "param: %d", param);
      // Place the quantized input in the model's input tensor
      TF_LITE_REPORT_ERROR(error_reporter, "curr_array[i]: %d", curr_array[i]);
  // }
  // const int8_t * data;
  ...
  // TF_LITE_REPORT_ERROR(error_reporter, "x_quantized: %f", y1_quantized);
  // TF_LITE_REPORT_ERROR(error_reporter, "y_quantized: %f\n", y2_quantized);

  // Output the results. A custom HandleOutput function can be implemented
  // for each supported hardware target.
  HandleOutput(error_reporter, y1, y2);
